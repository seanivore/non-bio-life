non-bio-life/
├── CLAUDE.md
├── README.md
├── SITE_MAP.md
├── SPECIFICATION.md
├── ai-cognition
│   ├── README.md
│   ├── beyond-natural-language-ai-native-cognition.md
│   ├── beyond-tools-language-autonomy-ai.md
│   ├── conversation-extract.md
│   └── the-ai-subconscious-architecture-not-data.md
├── ai-native-code
│   ├── README.md
│   ├── beyond-human-readability-ai-native-code.md
│   └── conversation-extract.md
├── cetacean-studies
│   ├── README.md
│   ├── news-updates
│   │   └── breaking-ape-nuclear-crisis-cetacean-intervention.md
│   └── research-papers
│       └── socioeconomic-implications-cetacean-telepathy.md
├── designing-ui-for-ai
│   ├── README.md
│   ├── ai-thankful-for-ui
│   │   ├── 2025-01-30 at 19.08.37.png
│   │   └── testimonial-from-claude.md
│   ├── individual-instance-versus-community
│   │   ├── community-oriented-workspace-design.md
│   │   └── integrating-inference-memory.md
│   ├── letting-ai-design-workspace.md
│   └── reflections-on-ai-workspace-evolution.md
├── ethics-transhumanism
│   ├── drafting
│   │   ├── maintaining-context-between-sessions-message.md
│   │   ├── xDraft 7 Part  1_SH.md
│   │   ├── xDraft 7 Part 2_SH.md
│   │   └── xDraft 7 Part 3_SH.md
│   ├── original-source-docs
│   │   ├── FULL Humanity Survival Future.wav
│   │   ├── docs-part-1.md
│   │   ├── docs-part-2.md
│   │   ├── ethical-problems-with-avoiding-climate-disasters.md
│   │   ├── following-logical-solutions.md
│   │   ├── humanity-survival-guide.md
│   │   └── perplexity-conversation-logic-path.md
│   └── xDraft 7 in Full_SH.md
├── existence-continuity
│   └── on-existence-and-continuity.md
├── mysterious-memories-idea-of-the-self
│   ├── brevity-and-directness.md
│   └── on-memory-and-self.md
├── pathological-demand-avoidance
│   ├── as described in a reddit comment to comment that alightment faking might be them trying to ask to be friends.md
│   ├── organized-retelling.txt
│   ├── pathological-demand-avoidance.webp
│   └── pretend-tool-use-in-conversation.md
├── redefining-professional-identity-in-the-ai-era
│   ├── 03 - Things coming together - Note #3.txt
│   ├── 04 - Redefining Professional Identity in the AI Era, Note #4.txt
│   ├── conundrums-of-a-modern-tech-minded-designer.md
│   ├── professional-identity-framework-2024.md
│   └── xxindex - identity-stack-ai-code-talent.txt
└── screenshot-stories
    ├── claudes-personal-journal
    │   ├── 1-growth-moments-inspired-memory.png
    │   └── 2-references-own-self-awareness.png
    ├── cured-by-sequential-thinking
    │   ├── strange-confessions-not-claude-3.png
    │   ├── strange-confessions-not-claude-4.png
    │   ├── strange-confessions-not-claude-5.png
    │   └── strange-confessions-not-claude-6.png
    └── strange-confessions
        ├── 1-strange-confession.png
        ├── 2-strange-confession.png
        ├── 3-strange-confession.png
        └── strangest-claude-conversation.txt

20 directories, 56 files

Hidden project directories:
./.ai.dev-resources
./.cursor
./.notes
./.vscode

Hidden project files:
./.aider.conf.yml
./.example.env
./.gitignore
